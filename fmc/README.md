...
1. FMC reads the manifest address of the Image Bundle, from the the HandOff Table and calculate Digest
1. FMC extends Caliptra PCR registers with TCI<sub>RT</sub>.
1. FMC extends Caliptra PCR registers with Image Manifest Digest
1. FMC derives CDI<sub>RT</sub> from CDI<sub>FMC</sub> mixed with TCI<sub>RT</sub> and Image Manifest Digest, then stores it in the Key Vault.
