...
pub fn make_fht(env: &RomEnv) -> FirmwareHandoffTable {
    let fht = FirmwareHandoffTable {
        marker: FHT_MARKER,
        major_ver: FHT_MAJOR_VERSION,
        minor_ver: FHT_MINOR_VERSION,
        manifest_base_addr: env.data_vault().map(|d| d.manifest_addr()),
        fips_fw_load_addr: u32::MAX,
        rt_fw_load_addr: env.data_vault().map(|d| d.rt_load_addr()),
        rt_fw_entry_point: env.data_vault().map(|d| d.rt_entry_point()),
