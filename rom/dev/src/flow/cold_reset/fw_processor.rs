...
impl FirmwareProcessor {
    ...
    fn retrieve_image_from_recovery_interface_proto(
        ...
        let dma_recovery = DmaRecovery::new(rri_base_addr, mci_base_addr, dma);
        cprintln!(
            "[fwproc] DMAing image to staging sram addr 0x{:08x}, Image Manifest Size {} byte",
            soc_ifc.staging_sram_addr(),
            size_of::<ImageManifest>()
        );
        dma_recovery.download_image_to_staging_sram(FW_IMAGE_INDEX, true, staging_sram_addr)
    }
}
