...
use caliptra_common::{
    ...
    keyids::{
        KEY_ID_FE, KEY_ID_LDEVID_CDI, KEY_ID_LDEVID_ECDSA_PRIV_KEY,
...
impl LocalDevIdLayer {
    ...
    pub fn derive(env: &mut RomEnv, input: &DiceInput) -> CaliptraResult<DiceOutput> {
        cprintln!("[ldev] ++");
        cprintln!("[ldev] CDI.KEYID = {}", KEY_ID_LDEVID_CDI as u8);
        cprintln!(
        ...
        Self::derive_stable_identity_root_idev(env, KEY_ID_LDEVID_CDI, KEY_ID_STABLE_IDEV)?;
        ...
        //
        // derive_cdi() takes one slot ID and uses it as both the source and destination.
        // Because the IDevID and LDevID CDIs are stored in the same slot, we just pass
        // the LDevID CDI slot here.
        Self::derive_cdi(env, KEY_ID_FE, KEY_ID_LDEVID_CDI)?;

        // Derive the stable identity root LDevID key
        Self::derive_stable_identity_root_ldev(env, KEY_ID_LDEVID_CDI, KEY_ID_STABLE_LDEV)?;
