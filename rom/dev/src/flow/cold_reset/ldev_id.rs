...
use caliptra_common::{
    ...
    keyids::{
        KEY_ID_FE, KEY_ID_IDEVID_CDI, KEY_ID_LDEVID_ECDSA_PRIV_KEY,
...
impl LocalDevIdLayer {
    ...
    pub fn derive(env: &mut RomEnv, input: &DiceInput) -> CaliptraResult<DiceOutput> {
        cprintln!("[ldev] ++");
        cprintln!("[ldev] CDI.KEYID = {}", KEY_ID_IDEVID_CDI as u8);
        cprintln!(
        ...
        Self::derive_stable_identity_root_idev(env, KEY_ID_IDEVID_CDI, KEY_ID_STABLE_IDEV)?;
        ...
        Self::derive_cdi(env, KEY_ID_FE, KEY_ID_IDEVID_CDI)?;

        // Derive the stable identity root LDevID key
        Self::derive_stable_identity_root_ldev(env, KEY_ID_IDEVID_CDI, KEY_ID_STABLE_LDEV)?;
