...
use caliptra_common::keyids::{
    KEY_ID_ECDSA_FE, KEY_ID_LDEVID_ECDSA_PRIV_KEY, KEY_ID_ROM_FMC_ECDSA_CDI,
};
...
impl LocalDevIdLayer {
    ...
    pub fn derive(env: &mut RomEnv, input: &DiceInput) -> CaliptraResult<DiceOutput> {
        cprintln!("[ldev] ++");
        cprintln!("[ldev] CDI.KEYID = {}", KEY_ID_ROM_FMC_ECDSA_CDI as u8);
        cprintln!(
            "[ldev] SUBJECT.KEYID = {}",
        ...
        cprintln!("[ldev] FE.KEYID = {}", KEY_ID_ECDSA_FE as u8);
        ...
        Self::derive_cdi(env, KEY_ID_ECDSA_FE, KEY_ID_ROM_FMC_ECDSA_CDI)?;

        // Derive DICE Key Pair from CDI
        let key_pair =
            Self::derive_key_pair(env, KEY_ID_ROM_FMC_ECDSA_CDI, KEY_ID_LDEVID_ECDSA_PRIV_KEY)?;
