...
| 08/15/2023 | 1.0     | Added LMS keys and signatures to image format                            |
...
| FUSE_KEY_MANIFEST_PK_HASH       | 384          | Hash of the four ECC and thiry-two LMS Manufacturer Public Keys   |
...
3.	**Data Vault**: Data Vault contains a set of sticky (lock till next cold reset), non-sticky (lock till next Warm reset) and scratch registers.
...
1. **STASH_MEASUREMENT**: Upto eight measurements can be sent to the ROM for recording. Format of a measurement is documented at https://github.com/chipsalliance/caliptra-sw/blob/main/runtime/README.md#stash_measurement
2. **VERSION**: [TODO] Add links to data structure formats once available.
3. **SELF_TEST_START**: This command is used to invoke the FIPS Knoww-Answer-Tests (aka KAT) on demand.  [TODO] Add links to data structure formats once available.
...
10.	Lock critical state needed for warm and update reset in Data Vault
    ...
    **Note**: A value of 0x140 is stored on a succesful cold boot.
...
- There are four ECC and thiry-two LMS manufacturingkeys in the preamble.
...
- Repeate the above procedure for LMS keys using the fuse_lms_revocation for key revocation.
