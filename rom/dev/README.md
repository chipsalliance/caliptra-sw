...
- Initiate image download from the recovery interface to the mailbox sram:
  ...
  - If the validation is succesful, set the `RECOVERY_STATUS` register `Device Recovery Status` field to 0x1 (`Awaiting recovery image`) and `DEVICE_STATUS` register, `Device Status` field to 0x5 (`Running Recovery Image ( Recover Reason Code not populated)`)
  - If the validation is fails, set the `RECOVERY_STATUS` register `Device Recovery Status` field to 0xc (`Recovery failed`) and and `DEVICE_STATUS` register, `Device Status` field to 0xF (`Fatal Error (Recover Reason Code not populated)`).
