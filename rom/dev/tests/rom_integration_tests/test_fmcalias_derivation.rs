...
use caliptra_image_gen::{ImageGenerator, ImageGeneratorVendorConfig};
...
fn test_fuse_log() {
    const FW_SVN: u32 = 4;
    const FW_SVN_FUSE: u32 = 3;

    let fuses = Fuses {
        fw_svn: [0x7, 0, 0, 0], // Value of FW_SVN_FUSE
        lms_verify: true,
        ..Default::default()
    };
    ...
    assert_eq!(fuse_log_entry.log_data[0], FW_SVN_FUSE);
