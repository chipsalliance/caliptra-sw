...
fn test_ocp_lock_flow() {
    ...
    let payload = MailboxReqHeader {
        chksum: caliptra_common::checksum::calc_checksum(u32::from(CommandId::TEST_OCP_LOCK), &[]),
    };

    let response = hw
        .mailbox_execute(CommandId::TEST_OCP_LOCK.into(), payload.as_bytes())
        .unwrap()
        .unwrap();

    let capabilities_resp = TestOcpLockResp::ref_from_bytes(response.as_bytes()).unwrap();
