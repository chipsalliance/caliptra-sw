...
use caliptra_image_fake_keys::{
    OWNER_LMS_KEY1_PUBLIC, VENDOR_CONFIG_KEY_0, VENDOR_CONFIG_KEY_1, VENDOR_CONFIG_KEY_2,
    VENDOR_CONFIG_KEY_3, VENDOR_LMS_KEY1_PUBLIC,
};
...
use caliptra_image_types::{
    ImageBundle, ImageLmsSignature, ImageManifest, OWNER_LMS_KEY_COUNT, VENDOR_ECC_KEY_COUNT,
    VENDOR_LMS_KEY_COUNT,
};
...
fn test_preamble_vendor_lms_pubkey_out_of_bounds() {
    ...
    assert_eq!(
        ModelError::MailboxCmdFailed(
            CaliptraError::IMAGE_VERIFIER_ERR_VEN_LMS_PUB_KEY_INDEX_OUT_OF_BOUNDS.into()
        ),
        hw.upload_firmware(&image_bundle.to_bytes().unwrap())
            .unwrap_err()
    );
...
fn test_header_verify_vendor_lms_sig_mismatch() {
    // first test to make sure it passes with the correct key
    let (mut hw, image_bundle) =
        helpers::build_hw_model_and_image_bundle(Fuses::default(), ImageOptions::default());
    assert!(hw
