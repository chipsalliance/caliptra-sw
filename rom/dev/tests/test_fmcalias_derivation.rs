...
use caliptra_common::{FirmwareHandoffTable, FuseLogEntry, FuseLogEntryId, StashMeasurement};
...
fn test_upload_single_measurement() {
    ...
    // Upload measurement.
    let measurement = StashMeasurement {
    ...
    // Get PCR31
    let result = hw.mailbox_execute(0x1000_0008, &[]);
    assert!(result.is_ok());
    ...
    let pcr_entry_arr = hw.mailbox_execute(0x1000_0009, &[]).unwrap().unwrap();
...
fn test_upload_measurement_limit() {
    ...
    let mut measurement = StashMeasurement {
        measurement: [0xdeadbeef; 12],
        ..Default::default()
    };
