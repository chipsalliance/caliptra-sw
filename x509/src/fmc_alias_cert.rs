...
mod tests {
    ...
    const TEST_TCB_INFO_SVN: &[u8] = &[0xB7];
    const TEST_TCB_INFO_MIN_SVN: &[u8] = &[0xB8];
    ...
    fn make_test_cert(subject_key: &Ecc384AsymKey, issuer_key: &Ecc384AsymKey) -> FmcAliasCertTbs {
        let params = FmcAliasCertTbsParams {
            ...
            tcb_info_svn: &TEST_TCB_INFO_SVN.try_into().unwrap(),
            tcb_info_min_svn: &TEST_TCB_INFO_MIN_SVN.try_into().unwrap(),
    ...
    fn test_extensions() {
        ...
        const TCB_INFO_OID: Oid = oid!(2.23.133 .5 .4 .1);
        assert!(ext_map[&TCB_INFO_OID].critical);
    }
}
