...
/*
#cgo CFLAGS: -I./../../inc -g
#cgo LDFLAGS: -L./  -lcaliptra1 -lcaliptra_hw_model_c_binding -ldl
...
func Commands(cmd []byte, n int) []byte {
	...
	// Call DPE GetProfile
	/*type DpeGetProfile struct {
		Magic   C.uint32_t
		Cmd     C.uint32_t
		Profile C.uint32_t
	}

	dpe_cmd := DpeGetProfile{
		Magic:   C.uint32_t(0x44504543),
		Cmd:     C.uint32_t(0x1),
		Profile: C.uint32_t(0),
	}
	req.data_size = C.uint32_t(unsafe.Sizeof(dpe_cmd))
	C.memcpy(unsafe.Pointer(&req.data[0]), unsafe.Pointer(&dpe_cmd), C.ulong(unsafe.Sizeof(dpe_cmd)))
	fmt.Println(req)*/

	// Convert cmd to a C-style byte array
	cCmd := C.CBytes(cmd)
	defer C.free(cCmd)
	...
	// Print the bytes of the selected union member
	fmt.Printf("Bytes of selected union member: %v\n", selectedBytes)

	/*dataSize := int(resp.data_size)
	dataSlice := C.GoBytes(unsafe.Pointer(&resp.get_profile[0]), C.int(dataSize))
	test := make([]byte, dataSize)
	copy(test, dataSlice)*/
