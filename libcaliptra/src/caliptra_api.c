...
bool caliptra_ready_for_firmware(void)
{
    ...
    do
    {
        ...
        if ((status & GENERIC_AND_FUSE_REG_CPTRA_FLOW_STATUS_READY_FOR_MB_PROCESSING_MASK) == GENERIC_AND_FUSE_REG_CPTRA_FLOW_STATUS_READY_FOR_MB_PROCESSING_MASK)
        {
            ready = true;
        }
        else
...
uint32_t caliptra_ready_for_runtime(void)
{
    ...
    do
    {
        ...
        if ((status & GENERIC_AND_FUSE_REG_CPTRA_FLOW_STATUS_READY_FOR_RUNTIME_MASK) == GENERIC_AND_FUSE_REG_CPTRA_FLOW_STATUS_READY_FOR_RUNTIME_MASK)
        {
            ready = true;
        }
        else
