...
pub trait SocManager {
    ...
    fn finish_mailbox_exec<'r>(
        ...
        Ok(Some(&resp_data.buffer[..resp_data.len]))
    }
    ...
    fn mailbox_exec_req<R: Request>(
        ...
        let Some(_) = SocManager::mailbox_exec(self, R::ID.into(), req.as_bytes(), packet)? else {
                return Err(CaliptraApiError::MailboxNoResponseData);
        };

        if packet.len < R::Resp::MIN_SIZE || packet.len > mem::size_of::<R::Resp>() {
            return Err(CaliptraApiError::MailboxUnexpectedResponseLen {
                ...
                actual: packet.len as u32,
            });
        }

        let mut response = R::Resp::new_zeroed();
        response.as_bytes_mut()[..packet.len].copy_from_slice(&packet.buffer[..packet.len]);

        let response_header = MailboxRespHeader::read_from_prefix(&mut packet.buffer[..]).unwrap();
        let actual_checksum = calc_checksum(0, &packet.buffer[4..packet.len]);
