...
impl HwModel for ModelFpgaSubsystem {
    ...
    fn warm_reset(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        self.step_until(|hw| {
            let finished_warm_reset = hw
                .mci_boot_milestones()
                .contains(McuBootMilestones::WARM_RESET_FLOW_COMPLETE);
            let cptra_fatal_error = hw.soc_ifc().cptra_fw_error_fatal().read() != 0;
            finished_warm_reset || cptra_fatal_error
        });

        let cptra_fatal_error = self.soc_ifc().cptra_fw_error_fatal().read();
        if cptra_fatal_error != 0 {
            Err(format!(
                "A Caliptra fatal error happened during warm reset: 0x{:08x}",
                cptra_fatal_error,
            ))?;
        }
