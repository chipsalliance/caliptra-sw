...
use xi3c::STEP_STATUS;
use zerocopy::IntoBytes;

// Global step status for debugging
...
const WAIT_I3C_SEND: Duration = Duration::from_micros(300);
...
impl XI3CWrapper {
    ...
    pub fn configure(&self) {
        writeln!(eoutput(), "I3C controller initializing").unwrap();
        ...
        writeln!(eoutput(), "I3C controller finished initializing").unwrap();
    }
...
impl ModelFpgaSubsystem {
    ...
    fn handle_log(&mut self) {
        ...
        if self.output().exit_requested() {
            writeln!(eoutput(), "Exiting firmware request").unwrap();
    ...
    fn bmc_step(&mut self) -> Result<()> {
        ...
        if !self.recovery_fifo_blocks.is_empty() {
            if let Some(last) = self.last_recovery_block_written {
                // 12.5 MHz
                // Packets are 256 bytes = 2048 bits
                // 163 microseconds to transmit
                // add some margin
            ...
            if !self.recovery_ctrl_written {
                self.last_recovery_block_written = Some(Instant::now());
                let status = self
                    .i3c_core()
                    .sec_fw_recovery_if()
                    .device_status_0()
    ...
    fn print_i3c_registers(&mut self) {
        writeln!(eoutput(), "Dumping registers").unwrap();
        writeln!(
            eoutput(),
    ...
    pub fn print_otp_memory(&self) {
        ...
        for (i, oi) in otp.iter().copied().enumerate() {
            if oi != 0 {
                writeln!(eoutput(), "OTP mem: {:03x}: {:02x}", i, oi).unwrap();
            }
        }
    }
...
impl Bus for FpgaRealtimeBus<'_> {
    fn read(&mut self, _size: RvSize, addr: RvAddr) -> Result<RvData, BusError> {
        if let Some(ptr) = self.ptr_for_addr(addr) {
            ...
            writeln!(eoutput(), "Error LoadAccessFault at address 0x{:x}", addr).unwrap();
