# Caliptra Runtime Firmware v2.1
...
`key_type = OwnerKey` is restricted to PL0. ROM identifies the pre-manifest PL0
caller using `MCI.MCU_LSU_AXI_USER`; Runtime uses the PL0 PAUSER authenticated
by the firmware manifest. The IDevID and LDevID key types remain available to
PL1 callers.
...
Programs Field Entropy (FE) into one of 4 FE partitions in the OTP fuse controller. This command
is typically used during device provisioning flows.

**Note**: This command is only available when the device is in the **Production** lifecycle state.

Command Code: `0x4645_5052` ("FEPR")

*Table: `FE_PROG` input arguments*
...
must solve the challenge by signing it with the appropriate ECC and MLDSA keys and returning
the result via the [`PRODUCTION_AUTH_DEBUG_UNLOCK_TOKEN`](#production_auth_debug_unlock_token)
command.

This command performs the same function as the ROM's `AUTH_DEBUG_UNLOCK_REQ` command
(documented in the [ROM README](../rom/dev/README.md#debug-unlock)), but is available at
runtime.

**Note**: This command requires the device to be in the **Production** lifecycle state and the
`PROD_DEBUG_UNLOCK_REQ` bit in the `SS_DBG_MANUF_SERVICE_REG_REQ` register to be set.

Command Code: `0x5044_5552` ("PDUR")

*Table: `PRODUCTION_AUTH_DEBUG_UNLOCK_REQ` input arguments*

| **Name**      | **Type** | **Description**
| --------      | -------- | ---------------
| chksum        | u32      | Checksum over other input arguments, computed by the caller. Little endian.
| length        | u32      | Length of the payload in DWORDs.
| unlock\_level | u8       | Debug unlock level (1–8).
| reserved      | u8[3]    | Reserved.

*Table: `PRODUCTION_AUTH_DEBUG_UNLOCK_REQ` output arguments*
...
Completes the production debug unlock handshake by validating the caller's solution to the
challenge generated by
[`PRODUCTION_AUTH_DEBUG_UNLOCK_REQ`](#production_auth_debug_unlock_req). The caller must
provide the device identifier and challenge from the previous request along with ECC-384 and
ML-DSA-87 signatures proving possession of the authorized debug unlock key.

Upon successful validation, Caliptra authorizes the debug unlock by setting the unlock level
in the `SS_SOC_DBG_UNLOCK_LEVEL` register.

This command performs the same function as the ROM's `AUTH_DEBUG_UNLOCK_TOKEN` command
(documented in the [ROM README](../rom/dev/README.md#debug-unlock)), but is available at
runtime.

**Note**: This command requires a preceding successful `PRODUCTION_AUTH_DEBUG_UNLOCK_REQ` in
the same boot session. The challenge is consumed on use and cannot be replayed.

Command Code: `0x5044_5554` ("PDUT")

*Table: `PRODUCTION_AUTH_DEBUG_UNLOCK_TOKEN` input arguments*
