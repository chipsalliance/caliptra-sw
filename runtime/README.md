...
## Mailbox Commands: Manifest-Based Image Authorization (1.2)

These commands are used by the [Manifest-Based Image Authorization system](#manifest-based-image-authorization-12).

### SET_AUTH_MANIFEST

The SoC uses this command to program an image manifest for Manifest-Based Image Authorization to Caliptra. In response to this command, the Caliptra Runtime will verify the manifest by authenticating the public keys and in turn using them to authenticate the IMC. On successful verification, the Runtime will store the IMEs into DCCM for future use.
