...
GET_IDEV_CSR is not exposed by runtime firmware.
...
GET_LDEV_CERT is not exposed by runtime firmware.
...
| chksum      | u32      | Checksum over other output arguments, computed by Caliptra. Little endian. |
| fips_status | u32      | Indicates if the command is FIPS approved or an error                      |
| dpe_result  | u32      | Result code of DPE DeriveChild command. Little endian.                     |
...
- INPUT_DATA = PCRX (RT journey PCR)
- TYPE = “RTJM”
- CONTEXT_HANDLE = Default context
- Set flag in the TCI Node that this node was created by the DPE implementation.
  This will be used to set the VENDOR_INFO field in TcbInfo to “VNDR”.

_Note: the Runtime CDI (from KeyVault) will be used as-needed and will not be
accessed during initialization._
