...
use crate::Drivers;
use caliptra_api::mailbox::GetAttestedEccCsrReq;
use caliptra_cfi_derive_git::cfi_impl_fn;
use caliptra_common::mailbox_api::{AttestedCsrResp, MailboxReqHeader, ResponseVarSize};
use caliptra_error::{CaliptraError, CaliptraResult};
use caliptra_x509::{LocalDevIdCsrTbsEcc384, LocalDevIdCsrTbsEcc384Params};
        ...
        let csr_len: usize = match self {
            DevIdKeyType::LdevId => {
                let params = LocalDevIdCsrTbsEcc384Params {
                    public_key: &[0u8; LocalDevIdCsrTbsEcc384Params::PUBLIC_KEY_LEN],
                    subject_sn: &[0u8; LocalDevIdCsrTbsEcc384Params::SUBJECT_SN_LEN],
                    ueid: &[0u8; LocalDevIdCsrTbsEcc384Params::UEID_LEN],
                };

                let csr = LocalDevIdCsrTbsEcc384::new(&params);
                let len = LocalDevIdCsrTbsEcc384::TBS_TEMPLATE_LEN;
                csr_data[..len].copy_from_slice(csr.tbs());

                len
            }
            DevIdKeyType::FmcAlias => {
                todo!("Generate attested CSR for FMC Alias key");
            }
            DevIdKeyType::RtAlias => {
                todo!("Generate attested CSR for RT Alias key");
            }
        };
...
impl AttestedEccCsrCmd {
    ...
    pub(crate) fn execute(
        ...
        let signed_eat_len = key_type.generate_attested_csr(
            drivers,
...
impl AttestedMldsaCsrCmd {
    ...
    pub(crate) fn execute(
        ...
        todo!("Implement attested MLDSA CSR generation logic");
    }
}
