...
use caliptra_drivers::{
    cmac_kdf, hmac_kdf, Aes, AesKey, AesOperation, Array4x16, Hmac, HmacKey, HmacMode, HmacTag,
...
impl MekSecret<'_> {
    ...
    fn derive_seed(self, aes: &mut Aes) -> CaliptraResult<MekSeed> {
        Ok(MekSeed(cmac_kdf(
            aes,
...
/// Checksum of an `Mek`
#[derive(Default, IntoBytes, KnownLayout, Immutable, ZeroizeOnDrop)]
...
impl OcpLockContext {
    ...
    pub fn derive_mek(
        ...
        let mek_secret = mek_secret_seed.derive(hmac, trng, kv)?;
        let mek_seed = mek_secret.derive_seed(aes)?;
        ...
        if !constant_time_eq(
            expect_mek_checksum.as_bytes(),
            MekChecksum::default().as_bytes(),
        ) && !constant_time_eq(expect_mek_checksum.as_bytes(), checksum.as_bytes())
        {
