...
use caliptra_common::mailbox_api::{
    GetImageInfoReq, GetImageInfoResp, MailboxResp,
    MailboxRespHeader,
};
...
pub const IMAGE_NOT_FOUND: u32 = 0x1; // FW ID not found in the image metadata entry collection.
pub const IMAGE_FOUND: u32 = 0x2; // FW ID found in the image metadata entry collection.
    ...
    pub(crate) fn get_image_metadata(
        ...
        if let Some(metadata_entry) =
            Self::find_metadata_entry(auth_manifest_image_metadata_col, cmd_fw_id) {
                return Ok(metadata_entry.clone());
            }
