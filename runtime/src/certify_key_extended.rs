...
use crate::{mldsa_dpe_env, mutrefbytes, with_dpe_env, Drivers, PauserPrivileges};
...
use dpe::{
    commands::{CertifyKeyMldsa87Cmd, CertifyKeyP384Cmd as CertifyKeyCmd, CommandExecution},
    response::Response,
...
impl CertifyKeyExtendedCmd {
    ...
    pub(crate) fn execute_mldsa87(
        ...
        let certify_key_cmd = CertifyKeyMldsa87Cmd::ref_from_bytes(&cmd.certify_key_req[..]).or(
            Err(CaliptraError::RUNTIME_DPE_COMMAND_DESERIALIZATION_FAILED),
        )?;

        let ueid = drivers.soc_ifc.fuse_bank().ueid();
        let mut env = mldsa_dpe_env(drivers, dmtf_device_info, None)?;
