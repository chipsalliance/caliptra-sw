...
impl KeyLadder {
    ...
    pub fn derive_secret(
        ...
        let key_ladder_svn = handoff.rt_min_svn();
        let key_ladder_kv = handoff.rt_key_ladder()?;
        ...
        let mut extend_ladder = |key_in: KeyId, key_out: KeyId| {
            Hmac::hmac_kdf(
                drivers,
                key_in,
                b"si_extend",
        ...
        let secret_source = if num_iters == 0 {
            ...
            extend_ladder(key_ladder_kv, KEY_ID_TMP)?;
            for _ in 1..num_iters {
                extend_ladder(KEY_ID_TMP, KEY_ID_TMP)?;
            }
            KEY_ID_TMP
        };
