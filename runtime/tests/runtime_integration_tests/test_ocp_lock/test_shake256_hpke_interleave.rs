...
use caliptra_api::mailbox::{
    CmShake256InitReq, CmShake256InitResp, CmShake256UpdateReq, CommandId, MailboxReq,
    OcpLockGenerateMpkResp, OCP_LOCK_WRAPPED_KEY_MAX_METADATA_LEN,
};
...
fn test_shake256_invalidated_by_hpke_operation() {
    ...
    // Step 1: Obtain a validated HPKE handle before starting the SHAKE256
    // session, so that HPKE key validation doesn't interfere with our test.
