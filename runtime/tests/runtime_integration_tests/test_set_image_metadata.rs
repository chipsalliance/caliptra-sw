...
use caliptra_auth_man_types::{
    ...
    AuthManifestSignatures,
};
use caliptra_common::mailbox_api::{
    CommandId, MailboxReq, MailboxReqHeader, SetAuthManifestReq, SetImageMetadataReq,
};
...
fn generate_image_metadata() -> AuthManifestImageMetadataWithSignatures {
...
fn test_metadata_expect_err(
    ...
    assert_error(model, expected_err, resp);
}

fn send_auth_manifest_cmd(payload: &[u8]) -> DefaultHwModel {
...
fn test_set_image_metadata_invalid_metadata_list_count() {
    let mut ims = generate_image_metadata();
    ims.image_metadata.header.entry_count = 0;
    test_metadata_expect_err(
        ims.as_bytes(),
...
fn test_set_image_metadata_insufficient_metadata_size() {
    ...
    ims.image_metadata.header.entry_count = ims.image_metadata.header.entry_count + 1;
    test_metadata_expect_err(
        ims.as_bytes()[..ims_size].as_ref(),
        CaliptraError::RUNTIME_AUTH_MANIFEST_IMAGE_METADATA_SET_INVALID_SIZE,
        None,
    );
}
...
fn test_set_image_metadata_invalid_vendor_metadata_ecc_sig() {
    let auth_man = generate_auth_manifest();
