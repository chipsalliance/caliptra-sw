...
fn test_export_cdi_destroyed_root_context() {
    ...
    // `fpga_subsystem` errors here because Core gets a fatal error during warm reset. We can ignore
    // any errors here so we can check for the fatal error afterwards.
    let _ = model.warm_reset_flow();
