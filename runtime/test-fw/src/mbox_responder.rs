...
use caliptra_common::{keyids::KEY_ID_TMP, mailbox_api::CommandId};
...
use caliptra_runtime::{
    handoff::RtHandoff, mailbox::Mailbox, ContextState, Crypto, DpeInstance, Drivers, U8Bool,
    MAX_HANDLES,
};
...
const OPCODE_READ_HASH_CHAIN_MAX_SVN: u32 = 0xD000_0000;
const OPCODE_READ_HASH_CHAIN_DIGEST: u32 = 0xE000_0000;
    ...
    loop {
        ...
        match cmd {
            ...
            CommandId(OPCODE_READ_HASH_CHAIN_DIGEST) => {
                ...
                if hash_chain_kv.is_err() {
                    // No hash chain was derived by FMC; signify with an empty response.
                    write_response(&mut drivers.mbox, &[]);
                } else {
                    let num_hashes = u32::read_from(read_request(&drivers.mbox)).unwrap();

                    let kv = hash_chain_kv.unwrap();
                    let hmac_input = if num_hashes == 0 {
                        kv
                    } else {
                        Crypto::hmac384_hash(&mut drivers, kv, KEY_ID_TMP).unwrap();
                        for _ in 1..num_hashes {
                            Crypto::hmac384_hash(&mut drivers, KEY_ID_TMP, KEY_ID_TMP).unwrap();
                        }
                        KEY_ID_TMP
                    };

                    let hmac_output =
                        Crypto::ecc384_hmac(&mut drivers, hmac_input, b"hmac_label", b"hmac_data")
                            .unwrap();
