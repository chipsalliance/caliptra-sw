...
use caliptra_runtime::{
    mailbox::Mailbox, ContextState, Drivers, RtBootStatus, TciMeasurement, U8Bool, MAX_HANDLES,
};
    ...
    loop {
        ...
        match cmd {
            ...
            CommandId(OPCODE_HASH_DPE_TCI_DATA) => {
                ...
                for context in drivers.persistent_data.get().dpe_state.contexts {
            ...
            CommandId(OPCODE_READ_DPE_ROOT_CONTEXT_MEASUREMENT) => {
                let root_idx =
                    Drivers::get_dpe_root_context_idx(&drivers.persistent_data.get().dpe_state)
                        .unwrap();
                let root_measurement = drivers.persistent_data.get().dpe_state.contexts[root_idx]
                    .tci
                    .tci_current
                    .as_bytes();
            ...
            CommandId(OPCODE_READ_DPE_INSTANCE) => {
                write_response(
                    &mut drivers.mbox,
                    drivers.persistent_data.get().dpe_state.as_bytes(),
                );
            }
            CommandId(OPCODE_CORRUPT_DPE_INSTANCE) => {
                ...
                drivers.persistent_data.get_mut().dpe_state = corrupted_dpe;
                write_response(&mut drivers.mbox, &[]);
            }
            ...
            CommandId(OPCODE_CORRUPT_DPE_ROOT_TCI) => {
                ...
                let root_idx =
                    Drivers::get_dpe_root_context_idx(&drivers.persistent_data.get().dpe_state)
                        .unwrap();
                drivers.persistent_data.get_mut().dpe_state.contexts[root_idx]
                    .tci
