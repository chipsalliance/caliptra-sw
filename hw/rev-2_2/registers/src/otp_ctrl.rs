...
impl<TMmio: caliptra_ureg::Mmio> RegisterBlock<TMmio> {
    ...
    /// Global volatile read lock for secret partition HW digest CSRs and DAI digest readback. W1S: writing 1 sets the lock bit, writing 0 leaves the bit unchanged, and only reset clears the register.
    ///
    /// Read value: [`otp_ctrl::regs::SecretDigestReadLockReadVal`]; Write value: [`otp_ctrl::regs::SecretDigestReadLockWriteVal`]
    #[inline(always)]
    pub fn secret_digest_read_lock(
        &self,
    ) -> caliptra_ureg::RegRef<crate::otp_ctrl::meta::SecretDigestReadLock, &TMmio> {
        unsafe {
            caliptra_ureg::RegRef::new_with_mmio(
    ...
    /// Volatile write lock for production vendor public key hashes. W1S: writing 1 sets a lock bit, writing 0 leaves the bit unchanged, and only reset clears the register. Bit i locks the corresponding PROD vendor public key hash entry, where bit 0 maps to CPTRA_CORE_VENDOR_PK_HASH_1.
    ///
    /// Read value: [`u32`]; Write value: [`u32`]
    #[inline(always)]
    ...
    /// Volatile write lock for the manufacturing vendor public key hash. W1S: writing 1 sets a lock bit, writing 0 leaves the bit unchanged, and only reset clears the register. Bit 0 locks CPTRA_CORE_VENDOR_PK_HASH_0 in VENDOR_HASHES_MANUF_PARTITION; bits 31:1 are reserved.
    ///
    /// Read value: [`u32`]; Write value: [`u32`]
    #[inline(always)]
    pub fn manuf_pk_hash_volatile_lock(
        &self,
    ) -> caliptra_ureg::RegRef<crate::otp_ctrl::meta::ManufPkHashVolatileLock, &TMmio> {
        unsafe {
            caliptra_ureg::RegRef::new_with_mmio(
    ...
    /// Volatile write lock for ratchet seed partitions. W1S: writing 1 sets a lock bit, writing 0 leaves the bit unchanged, and only reset clears the register. Bit i locks CPTRA_SS_LOCK_HEK_PROD_i.
    ///
    /// Read value: [`u32`]; Write value: [`u32`]
    #[inline(always)]
    ...
    pub fn cptra_ss_lock_hek_prod_7_digest(&self) -> CptraSsLockHekProd7DigestBlock<&TMmio> {
        CptraSsLockHekProd7DigestBlock {
            ptr: unsafe { self.ptr.add(0x1a4 / core::mem::size_of::<u32>()) },
            mmio: core::borrow::Borrow::borrow(&self.mmio),
        }
    }
}
...
impl CptraSsLockHekProd7Digest {
    pub const PTR: *mut u32 = 0x1a4 as *mut u32;
    /// # Safety
    ///
    /// Caller must ensure that all concurrent use of this
    /// peripheral in the firmware is done so in a compatible
...
pub mod regs {
    ...
    #[derive(Clone, Copy)]
    pub struct SecretDigestReadLockReadVal(u32);
    impl SecretDigestReadLockReadVal {
        /// Global volatile read lock for secret partition HW digest CSRs and DAI digest readback. W1S: writing 1 sets the lock bit, writing 0 leaves the bit unchanged, and only reset clears the register.
        #[inline(always)]
        ...
        /// Construct a WriteVal that can be used to modify the contents of this register value.
        #[inline(always)]
        pub fn modify(self) -> SecretDigestReadLockWriteVal {
            SecretDigestReadLockWriteVal(self.0)
        }
    }
    impl From<u32> for SecretDigestReadLockReadVal {
        #[inline(always)]
        fn from(val: u32) -> Self {
            Self(val)
        }
    }
    impl From<SecretDigestReadLockReadVal> for u32 {
        #[inline(always)]
        fn from(val: SecretDigestReadLockReadVal) -> u32 {
            val.0
        }
    }
    #[derive(Clone, Copy)]
    pub struct SecretDigestReadLockWriteVal(u32);
    impl SecretDigestReadLockWriteVal {
        /// Global volatile read lock for secret partition HW digest CSRs and DAI digest readback. W1S: writing 1 sets the lock bit, writing 0 leaves the bit unchanged, and only reset clears the register.
        #[inline(always)]
    ...
    impl From<u32> for SecretDigestReadLockWriteVal {
        #[inline(always)]
        fn from(val: u32) -> Self {
            Self(val)
        }
    }
    impl From<SecretDigestReadLockWriteVal> for u32 {
        #[inline(always)]
        fn from(val: SecretDigestReadLockWriteVal) -> u32 {
            val.0
        }
    }
    #[derive(Clone, Copy)]
...
pub mod meta {
    ...
    >;
    pub type SecretDigestReadLock = caliptra_ureg::ReadWriteReg32<
        0,
        crate::otp_ctrl::regs::SecretDigestReadLockReadVal,
        crate::otp_ctrl::regs::SecretDigestReadLockWriteVal,
    >;
    pub type VendorPkHashVolatileLock = caliptra_ureg::ReadWriteReg32<0, u32, u32>;
    pub type ManufPkHashVolatileLock = caliptra_ureg::ReadWriteReg32<0, u32, u32>;
