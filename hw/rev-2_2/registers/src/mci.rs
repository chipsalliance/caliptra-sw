...
impl<TMmio: caliptra_ureg::Mmio> RegisterBlock<TMmio> {
    ...
    /// MCU-ROM-driven debug intent. OR'd with the physical
    /// SS_DEBUG_INTENT strap to form the effective debug intent, so the
    /// MCU can assert debug intent via a register write instead of only
    /// the GPIO strap.
    /// [br]Write-1-set (W1S): a SW write of 1 sets the bit; writes of 0
    /// have no effect. Writable by MCU or the SoC Config Agent only
    /// before SS_CONFIG_DONE_STICKY is set.
    /// [br]Retained across warm reset; cleared only on cold reset.
    ///
    /// Read value: [`mci::regs::SsDebugIntentMcuReadVal`]; Write value: [`mci::regs::SsDebugIntentMcuWriteVal`]
    #[inline(always)]
    pub fn ss_debug_intent_mcu(
        &self,
    ) -> caliptra_ureg::RegRef<crate::mci::meta::SsDebugIntentMcu, &TMmio> {
        unsafe {
            caliptra_ureg::RegRef::new_with_mmio(
...
pub mod regs {
    ...
    #[derive(Clone, Copy)]
    pub struct SsDebugIntentMcuReadVal(u32);
    impl SsDebugIntentMcuReadVal {
        #[inline(always)]
        pub fn debug_intent(&self) -> bool {
            ((self.0 >> 0) & 1) != 0
        }
        /// Construct a WriteVal that can be used to modify the contents of this register value.
        #[inline(always)]
        pub fn modify(self) -> SsDebugIntentMcuWriteVal {
            SsDebugIntentMcuWriteVal(self.0)
        }
    }
    impl From<u32> for SsDebugIntentMcuReadVal {
        #[inline(always)]
        fn from(val: u32) -> Self {
            Self(val)
        }
    }
    impl From<SsDebugIntentMcuReadVal> for u32 {
        #[inline(always)]
        fn from(val: SsDebugIntentMcuReadVal) -> u32 {
            val.0
        }
    }
    #[derive(Clone, Copy)]
    pub struct SsDebugIntentMcuWriteVal(u32);
    impl SsDebugIntentMcuWriteVal {
        #[inline(always)]
        pub fn debug_intent(self, val: bool) -> Self {
            Self((self.0 & !(1 << 0)) | (u32::from(val) << 0))
        }
    }
    impl From<u32> for SsDebugIntentMcuWriteVal {
        #[inline(always)]
        fn from(val: u32) -> Self {
            Self(val)
        }
    }
    impl From<SsDebugIntentMcuWriteVal> for u32 {
        #[inline(always)]
        fn from(val: SsDebugIntentMcuWriteVal) -> u32 {
            val.0
        }
    }
    #[derive(Clone, Copy)]
...
pub mod meta {
    ...
    pub type SsDebugIntentMcu = caliptra_ureg::ReadWriteReg32<
        0,
        crate::mci::regs::SsDebugIntentMcuReadVal,
        crate::mci::regs::SsDebugIntentMcuWriteVal,
    >;
