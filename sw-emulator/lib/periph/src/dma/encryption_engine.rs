        ...
        if self.raise_fatal_error_flag {
            ...
            match Control::CMD::Value::try_from(self.command) {
                Ok(Control::CMD::Value::LOAD_MEK) => {
                    ...
                    } else if self.key_cache.contains_key(&self.metadata) {
                        self.error = Control::ERR::Value::VENDOR_SPECIFIC_REDUNDANT_METADATA;
                    } else {
                        self.key_cache
                            .insert(self.metadata, (self.auxiliary_data, self.mek));
                    }
                }
...
impl EncryptionEngine {
    ...
    fn write_mek(&mut self, _size: RvSize, index: usize, val: RvData) -> Result<(), BusError> {
        self.state_machine.context.mek[index] = val as u32;
        Ok(())
    }
    ...
    fn write_metadata(&mut self, _size: RvSize, index: usize, val: RvData) -> Result<(), BusError> {
        self.state_machine.context.metadata[index] = val as u32;
        Ok(())
    }

    fn write_aux(&mut self, _size: RvSize, index: usize, val: RvData) -> Result<(), BusError> {
        self.state_machine.context.auxiliary_data[index] = val as u32;
        Ok(())
    }
    ...
    fn write_control(&mut self, _size: RvSize, val: u32) -> Result<(), BusError> {
        ...
        let _ = match self.state_machine.state() {
            States::Ready => {
    ...
    fn read_control(&self, _size: RvSize) -> Result<u32, BusError> {
        ...
        // CMD bit
        value |= Control::CMD
            .val(self.state_machine.context.command as u32)
            .value;
...
mod tests {
    ...
    fn write_test_metadata(encryption_engine: &mut EncryptionEngine) {
        for i in 0..TEST_METD.len() {
            encryption_engine
                .write(RvSize::Word, METD + (i as u32) * 4, TEST_METD[i])
                .unwrap();
        }
    }

    fn write_test_mek_entry(encryption_engine: &mut EncryptionEngine) {
        for i in 0..TEST_MEK.len() {
            encryption_engine
                .write(RvSize::Word, MEK + (i as u32) * 4, TEST_MEK[i])
                .unwrap();
        }
        for i in 0..TEST_AUX.len() {
            encryption_engine
                .write(RvSize::Word, AUX + (i as u32) * 4, TEST_AUX[i])
                .unwrap();
        }
    ...
    fn test_encryption_engine_unload_mek_success() {
        ...
        assert_eq!(entry_count, 1);
        assert_eq!(ee.state_machine.context.key_cache.contains_key(meta), false);
    }
