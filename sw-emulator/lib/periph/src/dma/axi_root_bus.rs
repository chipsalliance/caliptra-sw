...
impl AxiRootBus {
    ...
    pub const MCU_SRAM_OFFSET_MINUS_1: AxiAddr = Self::MCI_OFFSET + 0x20_0000 - 1;
    ...
    pub fn write(&mut self, size: RvSize, addr: AxiAddr, val: RvData) -> Result<(), BusError> {
        match addr {
            ...
            Self::MCI_OFFSET..=Self::MCU_SRAM_OFFSET_MINUS_1 => {
                // TODO: ship this to an MCI peripheral, possibly in MCU
                Ok(())
            }
            Self::MCU_SRAM_OFFSET..=Self::MCU_SRAM_END => {
                if let Some(sender) = self.event_sender.as_mut() {
