...
pub struct RecoveryInterface {
    ...
    #[register_array(offset = 0x0a)]
    agent_capabilities: [u8; 2],
    ...
    hearthbeat: ReadOnlyRegister<u16>,
    ...
    cms_data: Vec<u8>, // TODO need a way to inject this
}
...
impl RecoveryInterface {
    ...
    pub fn new(cms_data: Vec<u8>) -> Self {
        Self {
            ...
            // heartbeat: ReadOnlyRegister::new(0),
    ...
    fn indirect_fifo_data_read(&mut self, size: RvSize) -> Result<RvData, BusError> {
        if size != RvSize::Word {
    ...
    fn indirect_fifo_ctrl_write(&mut self, size: RvSize, val: RvData) -> Result<(), BusError> {
        // Writes have to be Word aligned
        if size != RvSize::Word {
            Err(BusError::StoreAccessFault)?
        }
        let load: ReadWriteRegister<u32, IndirectCtrl::Register> = ReadWriteRegister::new(val);
        if load.reg.is_set(RESET) {
...
mod tests {
    ...
    fn test_get_image() {
        ...
        let mut rri = RecoveryInterface::new(image.clone());
