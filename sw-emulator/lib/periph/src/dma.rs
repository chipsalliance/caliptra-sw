...
impl Dma {
    ...
    fn key_vault_to_axi(&mut self) -> bool {
        ...
        // Key vault write route is hard-wired to the key vault 23.
        // Refer to `OCP_LOCK_KEY_RELEASE_KV_SLOT` defined in Caliptra RTL
...
mod tests {
    ...
    fn dma_read_u32(dma: &mut Dma, clock: &Clock, addr: AxiAddr) -> u32 {
        ...
        while {
            let status0 = dma
                .read(RvSize::Word, STATUS0_OFFSET, BusAccessType::DataLoad)
                .unwrap();
        ...
        dma.read(RvSize::Word, READ_DATA_OFFSET, BusAccessType::DataLoad)
            .unwrap()
    }
