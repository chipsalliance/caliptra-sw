...
impl HmacSha {
    ...
    pub fn on_write_control(&mut self, size: RvSize, val: RvData) -> Result<(), BusError> {
        ...
        println!("init, next, 512: {}, {}, {}",
            self.control.reg.is_set(Control::INIT),
            self.control.reg.is_set(Control::NEXT),
            self.control.reg.is_set(Control::MODE));
...
mod tests {
    ...
    #[test]
    fn test_hmac_sha512_multiblock() {
        // Use full 64-byte key for HMAC-512
        let mut key: [u8; 64] = [
            0xe1, 0xb5, 0x2c, 0x4f, 0xf8, 0xce, 0x9c, 0x4b, 0x60, 0xbd, 0x8e, 0xc7, 0x85, 0xab, 0x7b,
            0xf3, 0xdf, 0xfc, 0x70, 0x23, 0xf7, 0xc5, 0x15, 0x88, 0xf9, 0x6b, 0x94, 0xee, 0xba, 0x80,
            0xca, 0x3b, 0x9b, 0x9e, 0xd0, 0x5a, 0xb2, 0xac, 0x87, 0x97, 0xbb, 0x70, 0x39, 0xd6, 0x81,
            0xf2, 0xe4, 0x1f, 0xcf, 0xe6, 0xdd, 0xda, 0xb2, 0xe9, 0x51, 0x22, 0xd9, 0xc7, 0x16, 0xc2,
            0xb8, 0x40, 0x6b, 0xd4,
        ];

        // let data: [u8; 24] = [
        //     0x64, 0x20, 0x62, 0x79, 0x20, 0x74, 0x68, 0x65, 0x20, 0x48, 0x4d, 0x41, 0x43, 0x20, 0x61,
