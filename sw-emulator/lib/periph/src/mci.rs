...
impl Mci {
    ...
    pub fn cptra_request_mcu_reset(&self) {
        println!("[MCI] Requesting MCU reset");
            self.regs.borrow_mut().intr_block_rf_notif0_intr_trig_r |= NOTIF_CPTRA_MCU_RESET_REQ_STS_MASK;
...
impl Bus for Mci {
    ...
    fn register_outgoing_events(&mut self, sender: mpsc::Sender<Event>) {
        println!("[MCI] Registering outgoing events");
        self.regs.borrow_mut().register_outgoing_events(sender.clone());
    }
