...
pub struct Abr {
    ...
    kv_mlkem_sharedkey_wr_ctrl: ReadWriteRegister<u32, KeyWriteControl::Register>,
...
impl Abr {
    ...
    pub fn on_write_mlkem_kv_rd_seed_ctrl(
        ...
        if self.kv_mlkem_seed_rd_ctrl.reg.is_set(KvRdSeedCtrl::READ_EN) {
    ...
    pub fn on_write_mlkem_kv_rd_msg_ctrl(
        ...
        if self.kv_mlkem_msg_rd_ctrl.reg.is_set(KvRdSeedCtrl::READ_EN) {
    ...
    fn mlkem_write_sharedkey_to_keyvault(&mut self, key_id: u32, key_usage: u32) -> u32 {
        let shared_key = bytes_from_words_le(&self.mlkem_shared_key);
        let result = self.key_vault.write_key(key_id, &shared_key, key_usage);

        match result.err() {
            Some(BusError::LoadAccessFault)
            | Some(BusError::LoadAddrMisaligned)
            | Some(BusError::InstrAccessFault) => KvRdSeedStatus::ERROR::KV_READ_FAIL.value,
            Some(BusError::StoreAccessFault) | Some(BusError::StoreAddrMisaligned) => {
                KvRdSeedStatus::ERROR::KV_WRITE_FAIL.value
            }
            None => KvRdSeedStatus::ERROR::SUCCESS.value,
        }
    }
    ...
    fn mlkem_msg_read_complete(&mut self) {
        let key_id = self.kv_mlkem_msg_rd_ctrl.reg.read(KvRdSeedCtrl::READ_ENTRY);
        let msg_read_result = self.mlkem_read_msg_from_keyvault(key_id, false);

        self.kv_mlkem_msg_rd_status.reg.modify(
            KvRdSeedStatus::READY::SET
                + KvRdSeedStatus::VALID::SET
                + KvRdSeedStatus::ERROR.val(msg_read_result),
        );
    }
    ...
    fn mlkem_sharedkey_write_complete(&mut self) {
        let key_id = self
            .kv_mlkem_sharedkey_wr_ctrl
            .reg
            .read(KeyWriteControl::KEY_ID);
        let key_usage = self
            .kv_mlkem_sharedkey_wr_ctrl
            .reg
            .read(KeyWriteControl::USAGE);
        let write_result = self.mlkem_write_sharedkey_to_keyvault(key_id, key_usage);

        self.kv_mlkem_sharedkey_wr_status.reg.modify(
            KvRdSeedStatus::READY::SET
                + KvRdSeedStatus::VALID::SET
                + KvRdSeedStatus::ERROR.val(write_result),
        );
    }
    ...
    fn mlkem_encaps(&mut self) {
        ...
        match ek.encapsulate(&mut rng) {
            Ok((ct, ss)) => {
                // Store shared secret
                self.mlkem_shared_key =
                    words_from_bytes_le(&<[u8; 32]>::try_from(ss.as_slice()).unwrap());
