...
impl FipsTestHook {
    ...
    pub unsafe fn halt(hook_cmd: u8) {
    ...
    /// (Only when the hook_cmd matches the value from get_fips_test_hook_code)
    pub unsafe fn corrupt_data<T: core::marker::Copy>(hook_cmd: u8, data: &T) -> T {
        if get_fips_test_hook_code() == hook_cmd {
    ...
    /// (Only when the hook_cmd matches the value from get_fips_test_hook_code)
    pub unsafe fn update_hook_cmd(hook_cmd: u8, new_hook_cmd: u8) {
        if get_fips_test_hook_code() == hook_cmd {
