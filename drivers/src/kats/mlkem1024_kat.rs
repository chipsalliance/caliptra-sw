...
// Computed after corrupting first 4-byte of expected ciphertext
const KAT_IMPLICIT_REJECTION_SK_DIGEST: Array4x16 = Array4x16::new([
    0x0A93BDDE, 0x596B7198, 0xDC291AAB, 0x8A1CE69E, 0x468F7255, 0xC0B9AD38, 0x30B19523, 0x5E4CD0DB,
    0x61555123, 0xD23478D5, 0xD3EF4509, 0x20220C1A, 0xC6C704CD, 0x5BACF127, 0xB9798D1A, 0x9949F03C,
]);
...
fn kat_keygen_encaps_decaps(
    ...
    // Check decapsulation output
    let sk_dec_digest = sha2
        .sha512_digest(shared_key_dec.as_bytes())
        .map_err(|_| CaliptraError::KAT_SHA384_DIGEST_FAILURE)?;
    if sk_dec_digest != KAT_SK_DIGEST {
        Err(CaliptraError::KAT_MLKEM1024_SHARED_KEY_MISMATCH)?;
    }

    // Implicit rejection path
    ciphertext.0[0] = 0; // Corrupt the first 4-byte of ciphertext
    mlkem.decapsulate(
        &dk,
        &ciphertext,
        MlKem1024SharedKeyOut::Array(&mut shared_key_dec),
    ...
    let sk_dec_digest = sha2
        .sha512_digest(shared_key_dec.as_bytes())
        .map_err(|_| CaliptraError::KAT_SHA384_DIGEST_FAILURE)?;
    if sk_dec_digest != KAT_IMPLICIT_REJECTION_SK_DIGEST {
        Err(CaliptraError::KAT_MLKEM1024_SHARED_KEY_MISMATCH)?;
    }
