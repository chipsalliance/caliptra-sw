...
impl Hmac384<'_> {
    ...
    /// Pre-calculated KEY_SCHEDULE_CONTEXT_LEN
    const KEY_SCHEDULE_CONTEXT_LEN: usize = core::mem::size_of::<u8>() + (Self::NH * 2);
    ...
    pub fn combine_secrets<const NSECRET: usize>(
        ...
        let key_schedule_context = {
            // TODO(clundin): This may panic. Fix this once the code is linked to the main binary.
            let mut key_schedule_context = [0; Self::KEY_SCHEDULE_CONTEXT_LEN];
            let (mode, rem) = <u8>::mut_from_prefix(&mut key_schedule_context).unwrap();
            *mode = Mode::BASE.0;
            let (psk, rem) = <[u8; Hmac384::NH]>::mut_from_prefix(rem).unwrap();
            psk.clone_from_slice(&psk_id_hash);
            let info = <[u8; Hmac384::NH]>::mut_from_bytes(rem).unwrap();
            info.clone_from_slice(&info_id_hash);
