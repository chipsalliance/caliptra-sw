...
/// Used to read from an AXI address and DMA the contents to a variety of targets.
...
/// Used to write to an AXI address from a variety of origins.
...
impl Dma {
    ...
    pub fn setup_dma_write(&self, write_transaction: DmaWriteTransaction) {
        self.with_dma(|dma| {
            ...
            // Set the block size (always 0 for DMA writes).
            dma.block_size().write(|f| f.size(0));
...
impl<'a> DmaRecovery<'a> {
    ...
    const PROT_CAP2_HW_STATUS_SUPPORT: u32 = 0x200; // Bit 9 in agent_caps
    ...
    pub fn recovery_reason_from_firmware_verification_error(err: CaliptraError) -> u32 {
        match err {
            ...
            | CaliptraError::IMAGE_VERIFIER_ERR_UPDATE_RESET_FMC_LOAD_ADDR_MISMATCH => {
...
impl<'a> DmaEncryptionEngine<'a> {
    ...
    pub fn release_mek_from_key_vault(
        &self,
        mek_size: u32,
        key_vault_clear_callback: impl FnOnce() -> CaliptraResult<()>,
    ) -> CaliptraResult<()> {
        if mek_size > OCP_LOCK_ENCRYPTION_ENGINE_MAX_MEK_SIZE as u32 || mek_size & 0x3 != 0 {
            Err(CaliptraError::OCP_LOCK_ENGINE_INVALID_MEK_SIZE)?
        }
