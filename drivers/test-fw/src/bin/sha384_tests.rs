...
fn test_pcr_hash_extend_single_block() {
    ...
    let entry_id: [u8; 2] = [0x9c, 0x2f];
    let data: [u8; 46] = [
        0x48, 0x76, 0x0d, 0x13, 0xac, 0x42, 0xea, 0xd1, 0x96, 0xe5, 0x4d, 0xcb, 0xaa, 0x5e, 0x58,
        0x72, 0x06, 0x62, 0xa9, 0x6b, 0x91, 0x94, 0xe9, 0x81, 0x33, 0x29, 0xbd, 0xb6, 0x27, 0xc7,
        0xc1, 0xca, 0x77, 0x15, 0x31, 0x16, 0x32, 0xc1, 0x39, 0xe7, 0xa3, 0x59, 0x14, 0xfc, 0x1e,
        0xcd,
    ];
    pcr_bank.erase_all_pcrs();

    // Round 1: PCR is all zeros.
    let result = sha384.pcr_extend(PcrId::PcrId0, &entry_id, &data);
    assert!(result.is_ok());
    assert_eq!(
