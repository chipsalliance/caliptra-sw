...
use caliptra_drivers::{
    AxiAddr, Dma, DmaRecovery, Sha2_512_384, Sha2_512_384Acc, ShaAccLockState, SocIfc,
};
...
fn test_dma_sha384_mcu_sram() {
    ...
    // Create a copy with swapped endianness at the 4-byte boundaries
    // let mut test_data_swapped = [0u8; TEST_DATA_SIZE];
    // for (i, &word) in test_data.iter().enumerate() {
    //     let swapped_word = word.swap_bytes();
    //     let bytes = swapped_word.to_ne_bytes();
    //     test_data_swapped[i * 4..(i + 1) * 4].copy_from_slice(&bytes);
    // }
    // let test_data_bytes = &test_data_swapped;
    let test_data_bytes =
