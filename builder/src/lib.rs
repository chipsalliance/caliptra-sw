    ...
    match get_ci_rom_version() {
        CiRomVersion::Rom2_0_0 => {
            if rom_from_env == &firmware::ROM {
                ...
                Ok(include_bytes!(
                    "../../rom/ci_frozen_rom/2.0/caliptra-rom-with-log-2.0.0-62c8009.bin"
                )
                .as_slice()
                .into())
            } else {
        ...
        CiRomVersion::Rom2_0_1 => {
            if rom_from_env == &firmware::ROM {
                Ok(
                    include_bytes!("../../rom/ci_frozen_rom/2.0/caliptra-rom-2.0.1-3824083.bin")
                        .as_slice()
                        .into(),
                )
            } else if rom_from_env == &firmware::ROM_WITH_UART {
    ...
    match get_ci_rom_version() {
        CiRomVersion::Rom2_0_0 => {
            if rom_from_env == &firmware::ROM {
                ...
                Ok(include_bytes!(
                    "../../rom/ci_frozen_rom/2.0/caliptra-rom-fpga-2.0.0-62c8009.bin"
                )
                .as_slice()
                .into())
            } else if rom_from_env == &firmware::ROM_WITH_UART {
                ...
                Ok(include_bytes!(
                    "../../rom/ci_frozen_rom/2.0/caliptra-rom-fpga-with-log-2.0.0-62c8009.bin"
                )
                .as_slice()
                .into())
            } else {
        ...
        CiRomVersion::Rom2_0_1 => {
            if rom_from_env == &firmware::ROM {
                ...
                Ok(include_bytes!(
                    "../../rom/ci_frozen_rom/2.0/caliptra-rom-fpga-2.0.1-3824083.bin"
                )
                .as_slice()
                .into())
            } else if rom_from_env == &firmware::ROM_WITH_UART {
