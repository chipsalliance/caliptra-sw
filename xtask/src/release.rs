...
pub async fn release_checklist(tag_str: &str) -> Result<()> {
    ...
    match tag.component.as_str() {
        ...
        "rt" => {
            check_rt(&tag, &release_files)?;
        }
        // A combined runtime & fmc release is tagged with "fw"
        "fw" => {
            check_fmc(&tag, &release_files)?;
            check_rt(&tag, &release_files)?;
        }
    ...
    meta.check_nightly_workflow().await?;
...
impl ReleaseRelevantFiles {
    fn new() -> Result<Self> {
        ...
        let rt_readme = fs::read_to_string(PROJECT_ROOT.join("runtime/README.md"))?;
        Ok(Self {
            version_rs,
            common_rs,
...
fn check_rt(tag: &ReleaseTag, files: &ReleaseRelevantFiles) -> Result<()> {
    verify_common(tag, files, "RUNTIME", "u32", &files.rt_readme)?;
...
impl GitHubReleaseManager {
    ...
    async fn check_nightly_workflow(&self) -> Result<()> {
        info!(
        ...
        let url = format!(
            ...
            self.owner, self.repo, self.head_commit
        );
    ...
    async fn create_github_release(&self) -> Result<()> {
        info!(
        ...
        let release = self
            ...
            .body(&release_body)
            .draft(false)
            .prerelease(self.tag.rc.is_some())
            .send()
            .await?;
