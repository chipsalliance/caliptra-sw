...
func TestRunAll(t *testing.T) {
	// Caliptra 2.0 keeps the Go verification flow on the ECC profile. The
	// ML-DSA profile needs response/external-mu behavior that is not available
	// on this branch's dedicated ML-DSA hardware path.
	for _, p := range []DpeProfile{ProfileEcc384} {
		t.Run(p.String(), func(t *testing.T) {
			var d client.TestDPEInstance = &CptraModel{profile: p}
