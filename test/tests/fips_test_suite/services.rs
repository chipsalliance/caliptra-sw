...
use caliptra_builder::image_revision;
...
pub fn exec_fw_info<T: HwModel>(hw: &mut T) {
    let payload = MailboxReqHeader {
    ...
    // Verify command-specific response data
    assert_eq!(fw_info_resp.fmc_revision, image_revision().unwrap());
    assert_eq!(fw_info_resp.runtime_revision, image_revision().unwrap());
    assert_eq!(fw_info_resp.rom_revision, image_revision().unwrap());
