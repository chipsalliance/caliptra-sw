...
impl OcpLockKeyLadderBuilder {
    ...
    pub fn add_hek(self) -> Self {
        let hek: [u32; 16] = if matches!(
            caliptra_builder::get_ci_rom_version(),
            caliptra_builder::CiRomVersion::Rom2_1_0 | caliptra_builder::CiRomVersion::Rom2_1_1
        ) {
...
fn test_golden_ocp_lock_keyladder() {
    ...
    if is_rom_2_1 {
        ...
        assert_eq!(
            mek.mek,
            [
                94, 152, 39, 23, 80, 241, 238, 103, 219, 79, 3, 73, 159, 38, 155, 127, 106, 161,
                12, 18, 56, 249, 187, 171, 151, 33, 80, 177, 43, 88, 170, 232, 206, 234, 175, 214,
                95, 181, 46, 96, 36, 178, 8, 146, 11, 219, 238, 52, 72, 59, 214, 205, 102, 183, 43,
                144, 0, 226, 80, 160, 212, 180, 219, 171
            ]
        );
    } else {
        assert_eq!(
        ...
        assert_eq!(
            mek.mek,
            [
                235, 114, 154, 59, 138, 17, 5, 205, 58, 53, 45, 24, 253, 200, 171, 74, 184, 17,
                186, 43, 158, 48, 110, 1, 106, 105, 191, 44, 46, 195, 83, 105, 195, 151, 38, 91, 1,
                134, 10, 187, 197, 219, 237, 98, 58, 38, 97, 163, 221, 220, 4, 83, 33, 23, 201,
                172, 223, 16, 12, 7, 240, 156, 227, 208
            ]
        );
    }

    let encrypted_mpk = if is_rom_2_1 {
        EncryptedMpk {
            ...
            ct: vec![
                218, 219, 239, 125, 175, 23, 17, 207, 15, 19, 221, 63, 30, 192, 234, 45, 254, 19,
                63, 27, 63, 108, 28, 203, 11, 102, 222, 203, 23, 224, 168, 152,
            ],
            tag: vec![
        ...
        EncryptedMpk {
            ...
            ct: vec![
                220, 72, 67, 131, 218, 104, 31, 1, 0, 121, 122, 88, 196, 75, 47, 119, 64, 143, 219,
                79, 14, 166, 252, 194, 5, 93, 16, 186, 49, 62, 36, 62,
            ],
            tag: vec![
