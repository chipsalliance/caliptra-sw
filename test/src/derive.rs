...
use ml_dsa::{KeyGen, MlDsa87, VerifyingKey};
use openssl::{
    pkey::{PKey, Public},
    sha::{sha256, sha384},
};
...
impl IDevId {
    ...
    pub fn derive_mldsa_public_key(&self) -> VerifyingKey<MlDsa87> {
        let pair = ml_dsa::MlDsa87::key_gen_internal((&self.mldsa_seed).into());
        pair.verifying_key().clone()
    }
}
