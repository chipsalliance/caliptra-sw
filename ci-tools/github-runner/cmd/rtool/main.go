...
func main() {
	...
	case "download_artifact":
		if len(os.Args) <= 5 {
			log.Fatalf("usage: download_artifact <app_id> <installation_id> <workflow_filename> <artifact_name>")
		}
	...
	case "launch", "jitconfig", "publish_jitconfig":
		if len(os.Args) <= 4 {
		...
		case "publish_jitconfig":
			err = hello.PublishJitConfig(ctx, client, labels)
		case "jitconfig":
			if len(os.Args) <= 5 {
				log.Fatalf("usage: jitconfig <machine_type> <app_id> <installation_id > <runner_name>")
			}
