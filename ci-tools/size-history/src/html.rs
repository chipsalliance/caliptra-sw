...
<table>
  <tr><th>Commit</th><th>Author</th><th>Commit</th><th>ROM prod size</th><th>ROM with-uart size</th><th>FMC size</th><th>App size</th><th>App with OCP LOCK size</th></tr>
...
pub(crate) fn format_records(records: &[SizeRecord]) -> io::Result<String> {
    ...
    for record in records.iter().rev() {
        let ext_sizes = ExtendedSizes {
            ...
            app_size_with_uart_ocp_lock: ExtendedSizeInfo::from_change(
                last_sizes.app_size_with_uart_ocp_lock,
                record.sizes.app_size_with_uart_ocp_lock,
            ),
        };
...
fn is_important(sizes: &ExtendedSizes) -> bool {
    ...
    has_delta(&sizes.rom_size_prod)
        ...
        || has_delta(&sizes.app_size_with_uart_ocp_lock)
}
...
struct ExtendedSizes {
    ...
    app_size_with_uart_ocp_lock: Option<ExtendedSizeInfo>,
}
