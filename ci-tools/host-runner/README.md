...
This is a NixOS installation for Caliptra host runners. A host runnner is a Raspberry PI that oversees a cluster of FPGAs running in the Caliptra CI.
...
##  Flash SD Image
Create a new NixOS image. You should modify the flake target to the host your are bootstrapping. This example uses "caliptra-hostrunner0".
...
1. Install rtool
    - `(cd ci-tools/github-runner/cmd/rtool && go build .)`
...
caliptra-fpga-boss --zcu104 $ZCU_FTDI --sdwire $ZCU_SDWIRE serve $IMAGE -- rtool jitconfig "$FPGA_TARGET" "$APP_ID" "INSTALLATION_ID" "$IDENTIFIER-$RAND_POSTFIX"
```
